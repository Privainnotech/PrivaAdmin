...
router.get("/:QuotationId", async (req, res) => {
  try {
    ...
    let getRevise = await pool
      .request()
      .query(
        `SELECT QuotationRevised FROM Quotation WHERE QuotationId = ${QuotationId}`
      );
    let Revised = "";
    ...
    if (
      typeof quotations.recordset[0].QuotationPayTerm == "object" ||
      !quotations.recordset[0].QuotationPayTerm.includes("QuotationPayTerm")
    ) {
      quotations.recordset[0].QuotationPayTerm = "";
    } else {
      quotations.recordset[0].QuotationPayTerm = JSON.parse(
        quotations.recordset[0].QuotationPayTerm
      );
    }
    ...
    if (
      typeof quotations.recordset[0].QuotationDetail === "object" ||
      quotations.recordset[0].QuotationDetail === "null"
    ) {
      ...
      quotations.recordset[0].QuotationDetail = JSON.parse(
        quotations.recordset[0].QuotationDetail
      );
...
router.post("/add_pre_quotation", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .send({ message: "Successfully add Quotation", QuotationId });
  } catch (err) {
    res.status(500).send({ message: `${err}` });
  }
});
  ...
  try {
    ...
    if (ProductId == "null") {
      ...
      if (CheckProduct.recordset[0].check) {
        let getProductId = await pool
          .request()
          .query(
            `SELECT ProductId FROM MasterProduct WHERE ProductName = N'${SubItemName}'`
          );
...
router.delete("/delete_quotation/:QuotationId", async (req, res) => {
  try {
    let pool = await sql.connect(dbconfig);
    let QuotationId = req.params.QuotationId;
    let Status = await pool
      .request()
      .query(
        `SELECT QuotationStatus, QuotationRevised FROM Quotation WHERE QuotationId = ${QuotationId}`
      );
    if (Status.recordset[0].QuotationStatus == 1) {
      ...
      let selectItem = await pool
        .request()
        .query(
          `SELECT ItemId FROM QuotationItem WHERE QuotationId = ${QuotationId}`
        );
...
router.put("/edit_subitem/:SubItemId", async (req, res) => {
  try {
    let SubItemId = req.params.SubItemId;
    let { ProductId, SubItemName, SubItemPrice, SubItemQty, SubItemUnit } =
      req.body;
    ...
    if (checkProduct.recordset[0].check) {
      res
        .status(400)
        .send({ message: "Duplicate description with another product" });
      return;
    }
