...
router.get("/customer", async (req, res) => {
  try {
    let pool = await sql.connect(dbconfig);
    let SelectCustomer = `Select * FROM MasterCustomer WHERE CustomerActive = 1 order by CustomerName`;
...
router.get("/employee", async (req, res) => {
  try {
    let pool = await sql.connect(dbconfig);
    let SelectEmployee = `Select *, EmployeeTitle+EmployeeFname+' '+EmployeeLname EmployeeName FROM MasterEmployee WHERE EmployeeActive = 1 AND Authority = 1 order by EmployeeFname`;
...
router.get("/approver", async (req, res) => {
  try {
    let pool = await sql.connect(dbconfig);
    let SelectEmployee = `Select *, EmployeeTitle+EmployeeFname+' '+EmployeeLname EmployeeName FROM MasterEmployee WHERE EmployeeActive = 1 AND Approver = 1 order by EmployeeFname`;
    let Employee = await pool.request().query(SelectEmployee);
    res.status(200).send(JSON.stringify(Employee.recordset));
  } catch (err) {
    res.status(500).send({ message: `${err}` });
